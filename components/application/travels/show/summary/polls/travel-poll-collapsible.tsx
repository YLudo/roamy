import { deletePoll, votePoll } from "@/actions/polls";
...
import { Check, ChevronDown, ChevronUp, Loader2, Trash2, Users } from "lucide-react";
...
const TravelPollCollapsible = ({ poll }: { poll: IPoll }) => {
    ...
    const handleDelete = async () => {
        startTransition(async () => {
            const result = await deletePoll(poll.travelId, poll.id);

            if (result.error) {
                toast({
                    variant: "destructive",
                    title: "Suppression du sondage échouée !",
                    description: result.error,
                });
            } else {
                toast({
                    title: "Suppression du sondage réussie !",
                    description: "Votre sondage a été supprimé avec succès.",
                });
        ...
        <>
            ...
            {isDeletingModalOpen && (
                <DeletionModal
                    ...
                    description="Êtes-vous sûr de vouloir supprimer ce sondage ? Cette action est irréversible."
                    isLoading={isPending}
